...
void read_knapsack_input(const char *filename, int **weights, int **values, int *n)
{
...
int main()
{
    int *weights, *values, n;
    const int capacities[] = {1000, 2000, 4000, 8000, 10000};
    ...
    FILE *csv_file = fopen("results_varying_capacity.csv", "w");
    if (!csv_file)
    {
        printf("\033[1;31mError: Could not open results_varying_capacity.csv for writing\033[0m\n");
        exit(1);
    }

    // Write the header row to the CSV
    fprintf(csv_file, "File,Capacity,DP-BottomUp Avg Value,DP-BottomUp Avg Time (ms),DP-TopDown Avg Value,DP-TopDown Avg Time (ms),Greedy Avg Value,Greedy Avg Time (ms),GA Greedy Random Avg Value,GA Greedy Random within Time Avg Value,GA Greedy Random Avg Time (ms),GA Greedy Population Avg Value,GA Greedy Population Avg Time (ms)\n");
    ...
    for (int i = 0; i < num_files; i++)
    {
        ...
        read_knapsack_input(filenames[i], &weights, &values, &n);

        printf("\033[1;33mNumber of items:\033[0m %d\n", n);

        for (int k = 0; k < num_capacities; k++)
        {
            int W = capacities[k];
            printf("\033[1;33mTesting Capacity:\033[0m %d\n", W);
            ...
            printf("\033[1;32m[RESULT]\033[0m Capacity: %d, DP-BottomUp Avg Value: %d | Avg Time: %.3f ms\n", W, avg_value_dp_bu, avg_time_dp_bu);
            printf("\033[1;33m[RESULT]\033[0m Capacity: %d, DP-TopDown Avg Value: %d | Avg Time: %.3f ms\n", W, avg_value_dp_td, avg_time_dp_td);
            printf("\033[1;34m[RESULT]\033[0m Capacity: %d, Greedy Avg Value: %d | Avg Time: %.3f ms\n", W, avg_value_greedy, avg_time_greedy);
            printf("\033[1;36m[RESULT]\033[0m Capacity: %d, Greedy Random GA Avg Value: %d | within Time Value: %d | Avg Time: %.3f ms\n", W, avg_value_greedyrandom_ga, avg_timelimit_value_greedyrandom_ga, avg_time_greedyrandom_ga);
            printf("\033[1;36m[RESULT]\033[0m Capacity: %d, Greedy Population GA Avg Value: %d | Avg Time: %.3f ms\n", W, avg_value_greedypop_ga, avg_time_greedypop_ga);

            // Write results to CSV
            fprintf(csv_file, "%s,%d,%d,%.3f,%d,%.3f,%d,%.3f,%d,%d,%.3f,%d,%.3f\n",
                    filenames[i], W, avg_value_dp_bu, avg_time_dp_bu,
    ...
    fclose(csv_file);
    printf("\033[1;32mResults saved in 'results_varying_capacity.csv'\033[0m\n");
