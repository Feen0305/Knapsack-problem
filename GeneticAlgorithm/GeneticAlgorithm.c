...
Individual select_parent() {
    ...
    // Select a random point in the fitness space to choose a parent
...
void mutate(Individual *ind) {
    for (int i = 0; i < NUM_ITEMS; i++) {
        // Flip the gene (bit) with a probability of MUTATION_RATE
        // Condition is true if a random number divided by RAND_MAX is less than MUTATION_RATE
        // why not just use rand() < MUTATION_RATE * RAND_MAX?
        // because rand() returns a number between 0 and RAND_MAX, so the division is necessary
